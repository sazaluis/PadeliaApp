...
import { AlertTriangle, Check, Loader2, Calendar, ChevronRight, ChevronLeft, Clock, Repeat } from "lucide-react";
...
export function ApplyTemplateModal({
  ...
  // Auto-advance if template is preselected
  useEffect(() => {
  ...
  const dateRange = useMemo(
    () => getDateRange(mode, selectedYear, selectedMonth, selectedQuarter),
    [mode, selectedYear, selectedMonth, selectedQuarter]
  );
  ...
  // Check for existing sessions when entering step 3
  useEffect(() => {
    if (step === 3 && selectedTemplateId && dateRange) {
      const checkExisting = async () => {
        try {
          // We'll check on the backend; for now just set to 0
          // The backend will handle conflicts
          setExistingCount(0);
        } catch {}
      };
      checkExisting();
    }
  }, [step, selectedTemplateId, dateRange]);
  ...
  const renderStep1 = () => (
    <div className="space-y-4">
      <DialogDescription>
        Selecciona la plantilla de entrenamiento que deseas aplicar a un periodo completo.
      </DialogDescription>
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex gap-2 mt-1 text-xs text-muted-foreground">
                    ...
                    {t.coach && (
                      <span>
                        {t.coach.user.name} {t.coach.user.surname}
                      </span>
                    )}
                  </div>
                </div>
  ...
  const renderStep2 = () => (
    <div className="space-y-4">
      <DialogDescription>
        Elige el periodo al que deseas aplicar la plantilla{" "}
        <strong>
          {DAY_LABELS[selectedTemplate!.dayOfWeek]} {selectedTemplate!.startTime}
        </strong>{" "}
        del equipo <strong>{selectedTemplate!.team.name}</strong>.
      </DialogDescription>
      ...
      {/* Quarter selector (quarterly mode) */}
      {mode === "quarterly" && (
        <div>
      ...
      {dateRange && (
        <div className="rounded-lg bg-muted/50 p-3 space-y-1">
          ...
          <p className="text-sm text-muted-foreground">
            Se crearán <strong className="text-foreground">{sessionCount}</strong> sesiones
          </p>
          {hasPastDates && (
            <p className="text-xs text-amber-600 flex items-center gap-1 mt-1">
              <AlertTriangle className="h-3 w-3" />
              El periodo incluye fechas pasadas
            </p>
          )}
        </div>
      )}
  ...
  const renderStep3 = () => (
    <div className="space-y-4">
      {result ? (
        <>
          <div className="rounded-lg bg-primary/5 border border-primary/20 p-4 text-center">
            ...
            <div className="flex justify-center gap-4 mt-2 text-sm text-muted-foreground">
              <span>
                Creadas: <strong className="text-foreground">{result.created}</strong>
              </span>
              {result.skipped > 0 && (
                <span>
                  Omitidas: <strong className="text-foreground">{result.skipped}</strong>
                </span>
              )}
            </div>
          </div>
        ...
        <>
          <DialogDescription>
            Revisa los detalles antes de crear las sesiones de entrenamiento.
          </DialogDescription>
          <div className="rounded-lg border p-4 space-y-3">
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="text-sm font-medium">{selectedTemplate!.team.name}</span>
            </div>
            <div className="flex items-center justify-between">
          ...
          {hasPastDates && (
            <div className="flex items-start gap-2 rounded-lg bg-amber-50 border border-amber-200 p-3 text-sm text-amber-700">
              ...
              <span>
                El periodo seleccionado incluye fechas anteriores a hoy. Se crearán sesiones igualmente.
              </span>
            </div>
          )}
        </>
      ...
      {error && (
        <div className="text-sm text-destructive bg-destructive/5 p-3 rounded-md">{error}</div>
      )}
    </div>
  );

  const canProceedStep1 = selectedTemplateId !== "";
  const canProceedStep2 = dateRange !== null && sessionCount > 0;
